      ...
      <div style={{ overflowX: "auto", overflowY: "auto", flex: 1, minHeight: 0 }}>
        {vm.accountTab === "positions" ? (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ color: "#7e97a5", textAlign: "left" }}><th style={th}>{t.symbol}</th><th style={th}>{t.side}</th><th style={th}>{t.contracts}</th><th style={th}>{t.entry}</th><th style={th}>{t.mark}</th><th style={th}>{t.estimatedLiquidation}</th><th style={th}>{t.unrealizedPnl}</th><th style={th}>{t.action}</th></tr></thead>
            <tbody>{!state.position || state.position.side === "flat" ? <tr><td colSpan={8} style={{ padding: 18, color: "#60727f", textAlign: "center" }}>{t.noPosition}</td></tr> : <tr style={{ borderTop: "1px solid #13212a" }}><td style={td}>{state.position.symbol}</td><td style={{ ...td, color: state.position.side === "long" ? "#2dd4bf" : "#f87171" }}>{state.position.side}</td><td style={td}>{fmt(state.position.quantity, 4)}</td><td style={td}>{fmt(state.position.averageEntryPrice, vm.priceDigits)}</td><td style={td}>{fmt(state.position.markPrice, vm.priceDigits)}</td><td style={td}>{state.position.liquidationPrice > 0 ? fmt(state.position.liquidationPrice, vm.priceDigits) : "-"}</td><td style={{ ...td, color: state.position.unrealizedPnl > 0 ? "#2dd4bf" : state.position.unrealizedPnl < 0 ? "#f87171" : "#dbe7ef" }}>{fmt(state.position.unrealizedPnl, 4)} USDC</td><td style={td}><button onClick={() => void vm.closePosition()} style={btnInline}>{t.closePosition}</button></td></tr>}</tbody>
          </table>
